 ...
 */
public class CrabBoard {
	int width;
	int height;
	Random rand = new Random();
	public CrabPlayer crabPlayer;
	public List<CrabEnemy> crabEnemies = new ArrayList<CrabEnemy>();
	public List<CrabFriend> crabFriends = new ArrayList<CrabFriend>();
	 ...
	 */
	public CrabBoard(int width, int height){
		this.width = width;
		this.height = height;
		this.crabPlayer = new CrabPlayer(this);
	...
	public int checkSalinity(){
		...
		for (Rectangle r : scentTrail){
			if((r.getX() <= crabPlayer.getLocation().getX() + crabPlayer.getLocation().getWidth()) && r.getX() >= crabPlayer.getLocation().getX()){
				intersect = r.intersection(crabPlayer.getLocation());
	...
	public boolean checkCollision(){
		...
		for (Iterator<CrabEnemy> enemyIterator = crabEnemies.iterator(); enemyIterator.hasNext();){
			CrabEnemy crabEnemy = enemyIterator.next();
			
			if (crabPlayer.getLocation().intersects(crabEnemy.getLocation())){
				enemyIterator.remove();
				crabPlayer.setStarted(false);
	...
	public void moverUpdate(){
		...
		if (crabPlayer.getStarted()){
			...
			for(Iterator<CrabEnemy> enemyIterator = crabEnemies.iterator(); enemyIterator.hasNext();){
				CrabEnemy e = enemyIterator.next();				
			...
			for(Iterator<CrabFriend> friendIterator = crabFriends.iterator(); friendIterator.hasNext();){
				CrabFriend f = friendIterator.next();
			...
			if (!crabEnemies.isEmpty()){
				crabPlayer.setStarted(!checkCollision());
			}
		}else{
			setProgress(0);
		}
